...
\newcommand{\presource}[1]{\indicate{#1}\label{predef:#1}}
\newcommand{\note}[1]{\hyperref[def:#1]{#1}}
\newcommand{\inote}[1]{\hyperref[def:#1]{\indicate{#1}}}
\newcommand{\labelsection}[1]{\section{#1\label{sec:#1}}}
\newcommand{\labelsubsection}[1]{\subsection{#1\label{sec:#1}}}
\newcommand{\later}[1]{}
...
\newenvironment{toparent}{\begin{adjustwidth}{0.05\linewidth}{0.05\linewidth}\kaishu}
{\end{adjustwidth}}
...
\makeatletter
\newcounter{parfnote}
\renewcommand{\theparfnote}{\arabic{parfnote}}
\def\parfnotestore{}

% \newcommand{\@addparanote}[2]{\g@addto@macro\parfnotestore{\item[#1] #2}}

% \newcommand{\paranote}[1]{%
%     \stepcounter{parfnote}%
%     % 关键：先用 \edef 将 \theparfnote 展开为固定数字，存储在临时宏中
%     % 完全没用，原因不明
%     % 也不是完全没用，拿\number\value{parfnote}替换，空格会变大
%     \edef\@currentnum{\arabic{parfnote}}%
%     % 显示正文上标（使用同样的 \@currentnum）
%     \hbox{\@textsuperscript{\normalfont\theparfnote}}%
%     % 现在直接调用，不需要 \expandafter，因为 \@currentnum 已经是纯数字
%     % 还是需要的，不过因为不明原因，空格缩小了
%     % 最后发现，空格是因为 \@addparanote 命令换了行，把换行删掉就没有了
%     \expandafter\@addparanote\expandafter{\@currentnum}{#1}
% }

% 最后的解决方法是删掉所有换行。采用的代码是test_9/test_2里面的第一段代码。
\newcommand{\paranote}[1]{\stepcounter{parfnote}\textsuperscript{\theparfnote}\protected@edef\@tempa{\noexpand\g@addto@macro\noexpand\parfnotestore{\noexpand\item[\theparfnote] #1}}\@tempa}

\newcommand{\insertparanotes}{
    \ifx\parfnotestore\@empty\else
        \begin{center}\begin{minipage}{0.8\textwidth}
        \footnotesize\heiti%\songti
        \vspace{0.1em}\hrule\vspace{0.4em}
        \begin{list}{}{
...
% \backmatter
% \titlespacing{\chapter}{0cm}{11cm}{1cm}
% \include{summary}
